...
const UserForm: React.FC = () => {
	...
	const { users, handleCreateUser } = useAuthContext();
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const success = await handleCreateUser({ user: formData });
		if (success) {
			...
			<form onSubmit={handleSubmit} className='flex flex-col gap-5 bg-gray-50 shadow-md rounded-xl p-6'>
				...
				{!user ? (
					...
					<div className='flex justify-center items-center gap-3'>
						...
						<label className=' text-slate-700 italic font-semibold text-sm mb-1'>
							{formData.role.charAt(0) + formData.role.toLocaleLowerCase().slice(1)}
						</label>
					</div>
				)}
			</form>
